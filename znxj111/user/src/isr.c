...
//#include "PID.h"
#include "isr.h"
#include "my_true_control.h"
//#include "tongxun.h"
uint8_t Serial_RxFlag_1 = 0;
char Serial_RxPacket_1[100];
int zhuanxiang = 0;
...
void LPUART1_IRQHandler(void)
{
    if(kLPUART_RxDataRegFullFlag & LPUART_GetStatusFlags(LPUART1))
    {
        ...
        debug_interrupr_handler();                      // 调用 debug 串口接收处理函数 数据会被 debug 环形缓冲区读取
			...
			extern void OpenArtmini_uart_rx_interrupt_handler();
			OpenArtmini_uart_rx_interrupt_handler();
...
void LPUART4_IRQHandler(void)
{
    ...
    {
			extern void MCX_NEW_uart_rx_interrupt_handler();
			MCX_NEW_uart_rx_interrupt_handler();
        // 接收中断 
